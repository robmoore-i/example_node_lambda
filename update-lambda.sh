...
archive_zip_file="build/helloWorldWebsite-${timestamp}.zip"
source_files=`ls src`
...
aws lambda update-function-code --function-name helloWorldWebsite --zip-file fileb://$archive_zip_file  --publish
